...
import { addProductCard } from '../store/slices/cart.slice'
...
const handleClickPrevius = () =>{
  ...
  useEffect(() => {
    axiosEcommerce(`/products/${id}`).then((res) => setProduct(res.data))
  ...
  useEffect(() => {
    if(product){
      axiosEcommerce.get(`/products?categoryId=${product?.categoryId}`)
        .then((res) => {
          const newSimilarProducts = res.data.filter(peoductByCategory => peoductByCategory.id !== product.id)
